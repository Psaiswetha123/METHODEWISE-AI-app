...
class WizardController {
  constructor() {
    ...
    this.state = {
      ...
      description: 'Lightweight impact-resistant smart protective helmet with integrated Bluetooth & telemetry.',
      purpose: 'Protective Headwear for Sports & Micro-mobility',
  ...
  collectStepData(step) {
    if (step === 1) {
      this.state.productName = document.getElementById('prod-name')?.value || 'Smart Product';
      this.state.productType = document.getElementById('prod-type')?.value || 'Consumer Product';
  ...
  onDimensionChange() {
    ...
    // Update 3D canvas scale if active
    if (this.step2Viewer) {
  ...
  toggleMaterial(matId) {
    if (this.state.preferredMaterials.includes(matId)) {
      ...
      this.state.preferredMaterials = [matId]; // single select for clean clarity
    }
    this.renderMaterialsStep3();
  }
  ...
  submitWizard() {
    ...
    setTimeout(() => {
      ...
      if (window.app) {
        ...
        window.app.showToast('Analysis Completed Successfully.', 'success');
      }
    }, 1800);
  }
}
